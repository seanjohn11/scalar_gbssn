namespace Scalar_Evolution
{
  ...
  class Gamma_rr_Operator
  {
  public:
    static constexpr unsigned int n_quadrature_points_1d = n_points_1d;

    Gamma_rr_Operator(TimerOutput &timer_output);
  ...
  template <int dim, int degree, int n_points_1d>
  Gamma_rr_Operator<dim, degree, n_points_1d>::Gamma_rr_Operator(TimerOutput &timer)
    : timer(timer)
  {}
  ...
  // construction.
  template <int dim, int degree, int n_points_1d>
  void Gamma_rr_Operator<dim, degree, n_points_1d>::reinit(
  ...
  template <int dim, int degree, int n_points_1d>
  void Gamma_rr_Operator<dim, degree, n_points_1d>::initialize_vector(
  ...
  template <int dim, int degree, int n_points_1d>
  void Gamma_rr_Operator<dim, degree, n_points_1d>::local_apply_cell(
    ...
    for (unsigned int cell = cell_range.first; cell < cell_range.second; ++cell)
      {
        ...
        A_rr.reinit(cell);
        A_rr.gather_evaluate(A_rr_solution, EvaluationFlags::values);
        for (unsigned int q = 0; q < gamma_rr.n_q_points; ++q)
          {
            gamma_rr.submit_value(-2.*alpha.get_value(q)*A_rr.get_value(q)
            ,q);
          }
        gamma_rr.integrate_scatter(EvaluationFlags::values,dst);
      }
  }


  template <int dim, int degree, int n_points_1d>
  void Gamma_rr_Operator<dim, degree, n_points_1d>::local_apply_face(
    const MatrixFree<dim, Number> &,
    LinearAlgebra::distributed::Vector<Number> &      /*dst*/,
  ...
  template <int dim, int degree, int n_points_1d>
  void Gamma_rr_Operator<dim, degree, n_points_1d>::local_apply_boundary_face(
    const MatrixFree<dim, Number> &,
    LinearAlgebra::distributed::Vector<Number> &      /*dst*/,
    const LinearAlgebra::distributed::Vector<Number> &/*v_vec*/,
    const std::pair<unsigned int, unsigned int> &     /*face_range*/) const
  ...
  template <int dim, int degree, int n_points_1d>
  void Gamma_rr_Operator<dim, degree, n_points_1d>::local_apply_inverse_mass_matrix(
    const MatrixFree<dim, Number> &,
    LinearAlgebra::distributed::Vector<Number> &      dst,
    const LinearAlgebra::distributed::Vector<Number> &src,
    const std::pair<unsigned int, unsigned int> &     cell_range) const
  ...
  // parts.
  template <int dim, int degree, int n_points_1d>
  void Gamma_rr_Operator<dim, degree, n_points_1d>::apply(
  ...
  {
    {
      ...
      data.cell_loop(&Gamma_rr_Operator::local_apply_cell,
                //&Gamma_rr_Operator::local_apply_face,
                //&Gamma_rr_Operator::local_apply_boundary_face,
                this,
                dst,
                src,
                true/*,
...

    {
      ...
      data.cell_loop(&Gamma_rr_Operator::local_apply_inverse_mass_matrix,
                     this,
                     dst,
                     dst);
    }
  ...
  // update. It calls Gamma_rr_Operator::apply() followed by some updates
  ...
  template <int dim, int degree, int n_points_1d>
  void Gamma_rr_Operator<dim, degree, n_points_1d>::perform_stage(
    ...
    LinearAlgebra::distributed::Vector<Number> &      next_ri) const
  {
    ...
    //next_ri = 0;
    {
      ...
      data.cell_loop(
        &Gamma_rr_Operator::local_apply_inverse_mass_matrix,
        this,
        next_ri,
        vec_ki,
