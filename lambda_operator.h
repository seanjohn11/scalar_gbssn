namespace Scalar_Evolution
{
  ...
  class Lambda_Operator
  {
  public:
    static constexpr unsigned int n_quadrature_points_1d = n_points_1d;

    Lambda_Operator(TimerOutput &timer_output);
  ...
  template <int dim, int degree, int n_points_1d>
  Lambda_Operator<dim, degree, n_points_1d>::Lambda_Operator(TimerOutput &timer)
    : timer(timer)
  {}
  ...
  // construction.
  template <int dim, int degree, int n_points_1d>
  void Lambda_Operator<dim, degree, n_points_1d>::reinit(
  ...
  template <int dim, int degree, int n_points_1d>
  void Lambda_Operator<dim, degree, n_points_1d>::initialize_vector(
  ...
  template <int dim, int degree, int n_points_1d>
  void Lambda_Operator<dim, degree, n_points_1d>::local_apply_cell(
    const MatrixFree<dim, Number> &data,
    LinearAlgebra::distributed::Vector<Number> &      dst,
    const LinearAlgebra::distributed::Vector<Number> & src,
    const std::pair<unsigned int, unsigned int> &     cell_range) const
  {
    ...
    for (unsigned int cell = cell_range.first; cell < cell_range.second; ++cell)
      {
        lambda.reinit(cell);
        lambda.gather_evaluate(src, EvaluationFlags::values);
        conf.reinit(cell);
        ...
        K.reinit(cell);
        K.gather_evaluate(K_solution, EvaluationFlags::values|EvaluationFlags::gradients);
        diff_conf.reinit(cell);
        diff_conf.gather_evaluate(diff_conf_solution, EvaluationFlags::values);
        xi.reinit(cell);
        xi.gather_evaluate(xi_solution, EvaluationFlags::values);
        psi.reinit(cell);
        psi.gather_evaluate(psi_solution, EvaluationFlags::gradients);
        for (unsigned int q = 0; q < lambda.n_q_points; ++q)
          {VectorizedArray<double> r_val = lambda.quadrature_point(q)[0];

            lambda.submit_value(2.*alpha.get_value(q)*pow_f.eval(gamma_rr.get_value(q),-1)*
            (
              6.*A_tt.get_value(q)*conf.get_gradient(q)[0]*pow_f.eval(gamma_rr.get_value(q),-1)
              + 8.*numbers::PI*exp_f.eval(-4.*conf.get_value(q))*pow_f.eval(gamma_rr.get_value(q),-1)*xi.get_value(q)*pow_f.eval(gamma_tt.get_value(q),-1)*exp_f.eval(-4.*conf.get_value(q))*psi.get_gradient(q)[0]
              - 2./3.*K.get_gradient(q)[0]
            )
            + alpha.get_value(q)*pow_f.eval(gamma_rr.get_value(q),-1)*
            (
              A_rr.get_value(q)*pow_f.eval(gamma_rr.get_value(q),-2)*gamma_rr.get_gradient(q)[0]
              - 2.*A_tt.get_value(q)*pow_f.eval(gamma_tt.get_value(q),-2)*gamma_tt.get_gradient(q)[0]
              + 4.*A_tt.get_value(q)*pow_f.eval(gamma_tt.get_value(q)*gamma_tt.get_value(q)*r_val,-1)*(gamma_rr.get_value(q)-gamma_tt.get_value(q))
            )
            - 2.*A_rr.get_value(q)*pow_f.eval(gamma_rr.get_value(q),-2)*alpha.get_gradient(q)[0]
            ,q);

          }
        lambda.integrate_scatter(EvaluationFlags::values,dst);
      }
  }


  template <int dim, int degree, int n_points_1d>
  void Lambda_Operator<dim, degree, n_points_1d>::local_apply_face(
    ...
    for (unsigned int face = face_range.first; face < face_range.second; ++face)
      {
        ...
        A_rr_in.reinit(face);
        A_rr_in.gather_evaluate(A_rr_solution, EvaluationFlags::values);
        A_rr_out.reinit(face);
        A_rr_out.gather_evaluate(A_rr_solution, EvaluationFlags::values);
        A_tt_in.reinit(face);
        A_tt_in.gather_evaluate(A_tt_solution, EvaluationFlags::values);
        A_tt_out.reinit(face);
        A_tt_out.gather_evaluate(A_tt_solution, EvaluationFlags::values);


        for (unsigned int q = 0; q < lambda_in.n_q_points; ++q)
          {
            /*VectorizedArray<double> r_val = lambda_in.quadrature_point(q)[0];*/

            lambda_in.submit_value(
                                  (
                                    2.*alpha_in.get_value(q)*pow_f.eval(gamma_rr_in.get_value(q),-1)*
                                    (
                                      6.*A_tt_in.get_value(q)*pow_f.eval(gamma_rr_in.get_value(q),-1)*(0.5*(conf_in.get_value(q) + conf_out.get_value(q)) - conf_in.get_value(q))
                                      - 2./3.*(0.5*(K_in.get_value(q) + K_out.get_value(q)) - K_in.get_value(q))
                                    )
                                    + alpha_in.get_value(q)*pow_f.eval(gamma_rr_in.get_value(q),-1)*
                                    (
                                      A_rr_in.get_value(q)*pow_f.eval(gamma_rr_in.get_value(q),-2)*(0.5*(gamma_rr_in.get_value(q) + gamma_rr_out.get_value(q)) - gamma_rr_in.get_value(q))
                                      - 2.*A_tt_in.get_value(q)*pow_f.eval(gamma_tt_in.get_value(q),-2)*(0.5*(gamma_tt_in.get_value(q) + gamma_tt_out.get_value(q)) - gamma_tt_in.get_value(q))
                                    )
                                    - 2.*A_rr_in.get_value(q)*pow_f.eval(gamma_rr_in.get_value(q),-2)*(0.5*(alpha_in.get_value(q) + alpha_out.get_value(q)) - alpha_in.get_value(q))
                                  )*lambda_in.get_normal_vector(q)[0]
                              ,q);
            lambda_out.submit_value(
                                  -(
                                    2.*alpha_out.get_value(q)*pow_f.eval(gamma_rr_out.get_value(q),-1)*
                                    (
                                      6.*A_tt_out.get_value(q)*pow_f.eval(gamma_rr_out.get_value(q),-1)*(0.5*(conf_in.get_value(q) + conf_out.get_value(q)) - conf_out.get_value(q))
                                      - 2./3.*(0.5*(K_in.get_value(q) + K_out.get_value(q)) - K_out.get_value(q))
                                    )
                                    + alpha_out.get_value(q)*pow_f.eval(gamma_rr_out.get_value(q),-1)*
                                    (
                                      A_rr_out.get_value(q)*pow_f.eval(gamma_rr_out.get_value(q),-2)*(0.5*(gamma_rr_in.get_value(q) + gamma_rr_out.get_value(q)) - gamma_rr_out.get_value(q))
                                      - 2.*A_tt_out.get_value(q)*pow_f.eval(gamma_tt_out.get_value(q),-2)*(0.5*(gamma_tt_in.get_value(q) + gamma_tt_out.get_value(q)) - gamma_tt_out.get_value(q))
                                    )
                                    - 2.*A_rr_out.get_value(q)*pow_f.eval(gamma_rr_out.get_value(q),-2)*(0.5*(alpha_in.get_value(q) + alpha_out.get_value(q)) - alpha_out.get_value(q))
                                  )*lambda_in.get_normal_vector(q)[0]
                              ,q);

          }
        lambda_in.integrate_scatter(EvaluationFlags::values, dst);
        lambda_out.integrate_scatter(EvaluationFlags::values,dst);
  ...
  template <int dim, int degree, int n_points_1d>
  void Lambda_Operator<dim, degree, n_points_1d>::local_apply_boundary_face(
    const MatrixFree<dim, Number> &,
    LinearAlgebra::distributed::Vector<Number> &      dst,
    const LinearAlgebra::distributed::Vector<Number> &v_vec,
    const std::pair<unsigned int, unsigned int> &     face_range) const
  {
    ...
    for (unsigned int face = face_range.first; face < face_range.second; ++face)
      {
        lambda_in.reinit(face);
        lambda_in.gather_evaluate(v_vec, EvaluationFlags::values);
        conf_in.reinit(face);
        conf_in.gather_evaluate(conformal_solution, EvaluationFlags::values);
        K_in.reinit(face);
        K_in.gather_evaluate(K_solution, EvaluationFlags::values);
        ...
        for (unsigned int q = 0; q < lambda_in.n_q_points; ++q)
          {
            if (boundary_id == 0){
              ...
              lambda_in.submit_value(0.
                                ,q);
            }
            else if (boundary_id == 1){
              //lambda_in.submit_value(0.,q);
              lambda_in.submit_value(-lambda_in.get_value(q),q);
            }
  ...
  template <int dim, int degree, int n_points_1d>
  void Lambda_Operator<dim, degree, n_points_1d>::local_apply_inverse_mass_matrix(
    const MatrixFree<dim, Number> &,
    LinearAlgebra::distributed::Vector<Number> &      dst,
    const LinearAlgebra::distributed::Vector<Number> &src,
    const std::pair<unsigned int, unsigned int> &     cell_range) const
  ...
  // parts.
  template <int dim, int degree, int n_points_1d>
  void Lambda_Operator<dim, degree, n_points_1d>::apply(
  ...
  {
    {
      ...
      data.loop(&Lambda_Operator::local_apply_cell,
                &Lambda_Operator::local_apply_face,
                &Lambda_Operator::local_apply_boundary_face,
                this,
                dst,
                src,
                true/*,
...

    {
      ...
      data.cell_loop(&Lambda_Operator::local_apply_inverse_mass_matrix,
                     this,
                     dst,
                     dst);
    }
  ...
  // update. It calls Lambda_Operator::apply() followed by some updates
  ...
  template <int dim, int degree, int n_points_1d>
  void Lambda_Operator<dim, degree, n_points_1d>::perform_stage(
    ...
    LinearAlgebra::distributed::Vector<Number> &      next_ri) const
  {
    ...
    //next_ri = 0;
    {
      ...
      data.cell_loop(
        &Lambda_Operator::local_apply_inverse_mass_matrix,
        this,
        next_ri,
        vec_ki,
