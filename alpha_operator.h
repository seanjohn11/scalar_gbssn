namespace Scalar_Evolution
{
  ...
  class Alpha_Operator
  {
  public:
    static constexpr unsigned int n_quadrature_points_1d = n_points_1d;

    Alpha_Operator(TimerOutput &timer_output);
  ...
  template <int dim, int degree, int n_points_1d>
  Alpha_Operator<dim, degree, n_points_1d>::Alpha_Operator(TimerOutput &timer)
    : timer(timer)
  {}
  ...
  // construction.
  template <int dim, int degree, int n_points_1d>
  void Alpha_Operator<dim, degree, n_points_1d>::reinit(
  ...
  template <int dim, int degree, int n_points_1d>
  void Alpha_Operator<dim, degree, n_points_1d>::initialize_vector(
  ...
  template <int dim, int degree, int n_points_1d>
  void Alpha_Operator<dim, degree, n_points_1d>::local_apply_cell(
    const MatrixFree<dim, Number> &data,
    LinearAlgebra::distributed::Vector<Number> &      dst,
    const LinearAlgebra::distributed::Vector<Number> & src,
    const std::pair<unsigned int, unsigned int> &     cell_range) const
  {
    ...
    for (unsigned int cell = cell_range.first; cell < cell_range.second; ++cell)
      {
        alpha.reinit(cell);
        alpha.gather_evaluate(src, EvaluationFlags::values);
        K.reinit(cell);
        K.gather_evaluate(K_solution, EvaluationFlags::values);
        for (unsigned int q = 0; q < alpha.n_q_points; ++q)
          {/*VectorizedArray<double> r_val = alpha.quadrature_point(q)[0];*/

            alpha.submit_value(-2.*alpha.get_value(q)*K.get_value(q)
            ,q);

          }
        alpha.integrate_scatter(EvaluationFlags::values,dst);
      }
  }


  template <int dim, int degree, int n_points_1d>
  void Alpha_Operator<dim, degree, n_points_1d>::local_apply_face(
        ...
        for (unsigned int q = 0; q < xi_in.n_q_points; ++q)
          {
            ...
            xi_in.submit_value(alpha_in.get_value(q)*exp_f.eval(4.*conf_in.get_value(q))*
                               ...
                               (0.5*(diff_conf_in.get_value(q) + diff_conf_out.get_value(q)) - diff_conf_in.get_value(q))
            ...
            xi_out.submit_value(-alpha_out.get_value(q)*exp_f.eval(4.*conf_out.get_value(q))*
                               ...
                               (0.5*(diff_conf_in.get_value(q) + diff_conf_out.get_value(q)) - diff_conf_out.get_value(q))
                               *xi_in.get_normal_vector(q)[0]
                              ,q);
  ...
  template <int dim, int degree, int n_points_1d>
  void Alpha_Operator<dim, degree, n_points_1d>::local_apply_boundary_face(
    const MatrixFree<dim, Number> &,
    LinearAlgebra::distributed::Vector<Number> &      /*dst*/,
    const LinearAlgebra::distributed::Vector<Number> &/*v_vec*/,
    const std::pair<unsigned int, unsigned int> &     /*face_range*/) const
        ...
        for (unsigned int q = 0; q < xi_in.n_q_points; ++q)
          {
            if (boundary_id == 0){
              ...
              xi_in.submit_value(-alpha_in.get_value(q)*exp_f.eval(4.*conf_in.get_value(q))*
                                 gamma_tt_in.get_value(q)*gamma_tt_in.get_value(q)*pow_f.eval(gamma_rr_in.get_value(q),-1)*
                                 (0.5*(diff_conf_in.get_value(q)) - diff_conf_in.get_value(q))
                                ,q);
            }
  ...
  template <int dim, int degree, int n_points_1d>
  void Alpha_Operator<dim, degree, n_points_1d>::local_apply_inverse_mass_matrix(
    const MatrixFree<dim, Number> &,
    LinearAlgebra::distributed::Vector<Number> &      dst,
    const LinearAlgebra::distributed::Vector<Number> &src,
    const std::pair<unsigned int, unsigned int> &     cell_range) const
  ...
  // parts.
  template <int dim, int degree, int n_points_1d>
  void Alpha_Operator<dim, degree, n_points_1d>::apply(
  ...
  {
    {
      ...
      data.loop(&Alpha_Operator::local_apply_cell,
                &Alpha_Operator::local_apply_face,
                &Alpha_Operator::local_apply_boundary_face,
                this,
                dst,
                src,
                true/*,
...

    {
      ...
      data.cell_loop(&Alpha_Operator::local_apply_inverse_mass_matrix,
                     this,
                     dst,
                     dst);
    }
  ...
  // update. It calls Alpha_Operator::apply() followed by some updates
  ...
  template <int dim, int degree, int n_points_1d>
  void Alpha_Operator<dim, degree, n_points_1d>::perform_stage(
    ...
    LinearAlgebra::distributed::Vector<Number> &      next_ri) const
  {
    ...
    //next_ri = 0;
    {
      ...
      data.cell_loop(
        &Alpha_Operator::local_apply_inverse_mass_matrix,
        this,
        next_ri,
        vec_ki,
