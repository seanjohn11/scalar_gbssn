namespace Scalar_Evolution
{
  ...
  public:
    ...
    Psi_Operator(TimerOutput &timer_output);
  ...
  template <int dim, int degree, int n_points_1d>
  Psi_Operator<dim, degree, n_points_1d>::Psi_Operator(TimerOutput &timer)
    : timer(timer)
  {}
    ...
    const std::pair<unsigned int, unsigned int> &     cell_range) const
  {
    ...
    for (unsigned int cell = cell_range.first; cell < cell_range.second; ++cell)
      {
        psi.reinit(cell);
        psi.gather_evaluate(src, EvaluationFlags::values);
        conf.reinit(cell);
        conf.gather_evaluate(conformal_solution, EvaluationFlags::values);
        alpha.reinit(cell);
        alpha.gather_evaluate(alpha_solution, EvaluationFlags::values);
        gamma_rr.reinit(cell);
        gamma_rr.gather_evaluate(gamma_rr_solution, EvaluationFlags::values);
        gamma_tt.reinit(cell);
        gamma_tt.gather_evaluate(gamma_tt_solution, EvaluationFlags::values);
        xi.reinit(cell);
        xi.gather_evaluate(xi_solution, EvaluationFlags::values);
        for (unsigned int q = 0; q < psi.n_q_points; ++q)
          {/*VectorizedArray<double> r_val = psi.quadrature_point(q)[0];*/

            psi.submit_value(alpha.get_value(q)*exp_f.eval(-12.*conf.get_value(q))
                            *pow_f.eval(gamma_rr.get_value(q)*gamma_tt.get_value(q)*gamma_tt.get_value(q),-1)
                            *xi.get_value(q)
            ,q);
  ...
  // update. It calls Psi_Operator::apply() followed by some updates
