namespace Scalar_Evolution
{
  ...
  public:
    ...
    VOperator(TimerOutput &timer_output);
  ...
  template <int dim, int degree, int n_points_1d>
  VOperator<dim, degree, n_points_1d>::VOperator(TimerOutput &timer)
    : timer(timer)
  {}
  ...
  // construction.
  template <int dim, int degree, int n_points_1d>
  void VOperator<dim, degree, n_points_1d>::reinit(
  ...
  template <int dim, int degree, int n_points_1d>
  void VOperator<dim, degree, n_points_1d>::initialize_vector(
  ...
  template <int dim, int degree, int n_points_1d>
  void VOperator<dim, degree, n_points_1d>::local_apply_cell(
    const MatrixFree<dim, Number> &data,
    LinearAlgebra::distributed::Vector<Number> &      dst,
    ...
    const std::pair<unsigned int, unsigned int> &     cell_range) const
  {
    ...
    for (unsigned int cell = cell_range.first; cell < cell_range.second; ++cell)
      {
        v.reinit(cell);
        //v.gather_evaluate(v_vec, EvaluationFlags::values);
        conf.reinit(cell);
        ...
        for (unsigned int q = 0; q < v.n_q_points; ++q)
          {VectorizedArray<double> r_val = v.quadrature_point(q)[0];

            v.submit_value(exp_f.eval(4.*conf.get_value(q))*(
              2.*pow_f.eval(r_val,-1)*diff_conf.get_value(q)
              + conf.get_gradient(q)[0]*diff_conf.get_value(q)
              + diff_conf.get_gradient(q)[0]
            )
            + H_f.value(v.quadrature_point(q))
            ,q);

          }
        v.integrate_scatter(EvaluationFlags::values,dst);
      }
  }


  template <int dim, int degree, int n_points_1d>
  void VOperator<dim, degree, n_points_1d>::local_apply_face(
    ...
    const std::pair<unsigned int, unsigned int> &     face_range) const
  {
    ...
    FEFaceEvaluation<dim, degree, alt_q_points, 1, Number> v_in(data, true,0);
    FEFaceEvaluation<dim, degree, alt_q_points, 1, Number> v_out(data, false, 0);
    ...
    for (unsigned int face = face_range.first; face < face_range.second; ++face)
      {
        ...
        for (unsigned int q = 0; q < v_in.n_q_points; ++q)
          {
            ...
            v_in.submit_value(exp_f.eval(conf_in.get_value(q))*(0.5*diff_conf_avg - tau_val*conf_jump - diff_conf_in.get_value(q))*v_in.get_normal_vector(q),q);
            v_out.submit_value(-exp_f.eval(conf_out.get_value(q))*(0.5*diff_conf_avg + tau_val*conf_jump - diff_conf_out.get_value(q))*v_in.get_normal_vector(q),q);

          }
        v_in.integrate_scatter(EvaluationFlags::values, dst);
        v_out.integrate_scatter(EvaluationFlags::values,dst);
  ...
  template <int dim, int degree, int n_points_1d>
  void VOperator<dim, degree, n_points_1d>::local_apply_boundary_face(
    const MatrixFree<dim, Number> &,
    LinearAlgebra::distributed::Vector<Number> &      dst,
    const LinearAlgebra::distributed::Vector<Number> &v_vec,
    const std::pair<unsigned int, unsigned int> &     face_range) const
    ...
    for (unsigned int face = face_range.first; face < face_range.second; ++face)
      {
        ...
        for (unsigned int q = 0; q < v_in.n_q_points; ++q)
          {
            if (boundary_id == 0){
              //v_in.submit_value(0.,q);
              v_in.submit_value(-exp_f.eval(conf_in.get_value(q))*(0.5*diff_conf_in.get_value(q) - diff_conf_in.get_value(q)),q);
            }
            else if (boundary_id == 1){
              //v_in.submit_value(0.,q);
              v_in.submit_value(-v_in.get_value(q),q);
            }
  ...
  template <int dim, int degree, int n_points_1d>
  void VOperator<dim, degree, n_points_1d>::local_apply_inverse_mass_matrix(
    const MatrixFree<dim, Number> &,
    LinearAlgebra::distributed::Vector<Number> &      dst,
    const LinearAlgebra::distributed::Vector<Number> &src,
    const std::pair<unsigned int, unsigned int> &     cell_range) const
  ...
  // parts.
  template <int dim, int degree, int n_points_1d>
  void VOperator<dim, degree, n_points_1d>::apply(
  ...
  {
    {
      ...
      data.loop(&VOperator::local_apply_cell,
                &VOperator::local_apply_face,
                &VOperator::local_apply_boundary_face,
                this,
                dst,
                src,
                true/*,
...

    {
      ...
      data.cell_loop(&VOperator::local_apply_inverse_mass_matrix,
                     this,
                     dst,
                     dst);
    }
  ...
  // update. It calls VOperator::apply() followed by some updates
  ...
  template <int dim, int degree, int n_points_1d>
  void VOperator<dim, degree, n_points_1d>::perform_stage(
    ...
    LinearAlgebra::distributed::Vector<Number> &      next_ri) const
  {
    ...
    //next_ri = 0;
    {
      ...
      data.cell_loop(
        &VOperator::local_apply_inverse_mass_matrix,
        this,
        next_ri,
        vec_ki,
