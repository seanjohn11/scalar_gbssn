namespace Scalar_Evolution
{
  ...
  public:
    ...
    Xi_Operator(TimerOutput &timer_output);
  ...
  template <int dim, int degree, int n_points_1d>
  Xi_Operator<dim, degree, n_points_1d>::Xi_Operator(TimerOutput &timer)
    : timer(timer)
  {}
  ...
  // construction.
  template <int dim, int degree, int n_points_1d>
  void Xi_Operator<dim, degree, n_points_1d>::reinit(
  ...
  template <int dim, int degree, int n_points_1d>
  void Xi_Operator<dim, degree, n_points_1d>::initialize_vector(
  ...
  template <int dim, int degree, int n_points_1d>
  void Xi_Operator<dim, degree, n_points_1d>::local_apply_cell(
    const MatrixFree<dim, Number> &data,
    LinearAlgebra::distributed::Vector<Number> &      dst,
    const LinearAlgebra::distributed::Vector<Number> & src,
    const std::pair<unsigned int, unsigned int> &     cell_range) const
  {
    ...
    FEEvaluation<dim, degree, alt_q_points, 1, Number> diff_psi(data,0);
    ...
    for (unsigned int cell = cell_range.first; cell < cell_range.second; ++cell)
      {
        xi.reinit(cell);
        xi.gather_evaluate(src, EvaluationFlags::values);
        conf.reinit(cell);
        ...
        diff_psi.reinit(cell);
        diff_psi.gather_evaluate(diff_psi_solution, EvaluationFlags::values | EvaluationFlags::gradients);
        for (unsigned int q = 0; q < xi.n_q_points; ++q)
          {VectorizedArray<double> r_val = xi.quadrature_point(q)[0];

            xi.submit_value(alpha.get_value(q)*exp_f.eval(4.*conf.get_value(q))
                            *pow_f.eval(gamma_rr.get_value(q),-1)*gamma_tt.get_value(q)*gamma_tt.get_value(q)*
                            (
                              diff_psi.get_gradient(q)[0]
                              + 2.*diff_psi.get_value(q)*pow_f.eval(r_val,-1)
                              + diff_psi.get_value(q)*
                              (
                                alpha.get_gradient(q)[0]*pow_f.eval(alpha.get_value(q),-1)
                                + 2.*gamma_rr.get_gradient(q)[0]*pow_f.eval(gamma_rr.get_value(q),-1)
        ...
        xi.integrate_scatter(EvaluationFlags::values,dst);
      }
  }


  template <int dim, int degree, int n_points_1d>
  void Xi_Operator<dim, degree, n_points_1d>::local_apply_face(
    ...
    const std::pair<unsigned int, unsigned int> &     face_range) const
  {
    ...
    FEFaceEvaluation<dim, degree, alt_q_points, 1, Number> xi_in(data, true,0);
    FEFaceEvaluation<dim, degree, alt_q_points, 1, Number> xi_out(data, false, 0);
    ...
    FEFaceEvaluation<dim, degree, alt_q_points, 1, Number> diff_psi_in(data,true, 0);
    FEFaceEvaluation<dim, degree, alt_q_points, 1, Number> diff_psi_out(data,false, 0);
    ...
    for (unsigned int face = face_range.first; face < face_range.second; ++face)
      {
        ...
        diff_psi_in.reinit(face);
        diff_psi_in.gather_evaluate(diff_psi_solution, EvaluationFlags::values);
        diff_psi_out.reinit(face);
        diff_psi_out.gather_evaluate(diff_psi_solution, EvaluationFlags::values);
        ...
        for (unsigned int q = 0; q < xi_in.n_q_points; ++q)
          {
            /*VectorizedArray<double> r_val = xi_in.quadrature_point(q)[0];*/

            xi_in.submit_value(alpha_in.get_value(q)*exp_f.eval(4.*conf_in.get_value(q))*
                               gamma_tt_in.get_value(q)*gamma_tt_in.get_value(q)*pow_f.eval(gamma_rr_in.get_value(q),-1)*
                               (0.5*(diff_psi_in.get_value(q) + diff_psi_out.get_value(q)) - diff_psi_in.get_value(q))
                               *xi_in.get_normal_vector(q)[0]
                              ,q);
            xi_out.submit_value(-alpha_out.get_value(q)*exp_f.eval(4.*conf_out.get_value(q))*
                               gamma_tt_out.get_value(q)*gamma_tt_out.get_value(q)*pow_f.eval(gamma_rr_out.get_value(q),-1)*
                               (0.5*(diff_psi_in.get_value(q) + diff_psi_out.get_value(q)) - diff_psi_out.get_value(q))
                               *xi_in.get_normal_vector(q)[0]
                              ,q);

          }
        xi_in.integrate_scatter(EvaluationFlags::values, dst);
        xi_out.integrate_scatter(EvaluationFlags::values,dst);
  ...
  template <int dim, int degree, int n_points_1d>
  void Xi_Operator<dim, degree, n_points_1d>::local_apply_boundary_face(
    const MatrixFree<dim, Number> &,
    LinearAlgebra::distributed::Vector<Number> &      dst,
    const LinearAlgebra::distributed::Vector<Number> &v_vec,
    const std::pair<unsigned int, unsigned int> &     face_range) const
  {
    ...
    for (unsigned int face = face_range.first; face < face_range.second; ++face)
      {
        xi_in.reinit(face);
        xi_in.gather_evaluate(v_vec, EvaluationFlags::values);
        conf_in.reinit(face);
        conf_in.gather_evaluate(conformal_solution, EvaluationFlags::values);
        diff_psi_in.reinit(face);
        diff_psi_in.gather_evaluate(diff_psi_solution, EvaluationFlags::values);
        ...
        for (unsigned int q = 0; q < xi_in.n_q_points; ++q)
          {
            if (boundary_id == 0){
              ...
              xi_in.submit_value(-alpha_in.get_value(q)*exp_f.eval(4.*conf_in.get_value(q))*
                                 gamma_tt_in.get_value(q)*gamma_tt_in.get_value(q)*pow_f.eval(gamma_rr_in.get_value(q),-1)*
                                 (0.5*(diff_psi_in.get_value(q) + diff_psi_in.get_value(q)) - diff_psi_in.get_value(q))
                                ,q);
            }
            else if (boundary_id == 1){
              //xi_in.submit_value(0.,q);
              xi_in.submit_value(-xi_in.get_value(q),q);
            }
  ...
  template <int dim, int degree, int n_points_1d>
  void Xi_Operator<dim, degree, n_points_1d>::local_apply_inverse_mass_matrix(
    const MatrixFree<dim, Number> &,
    LinearAlgebra::distributed::Vector<Number> &      dst,
    const LinearAlgebra::distributed::Vector<Number> &src,
    const std::pair<unsigned int, unsigned int> &     cell_range) const
  ...
  // parts.
  template <int dim, int degree, int n_points_1d>
  void Xi_Operator<dim, degree, n_points_1d>::apply(
  ...
  {
    {
      ...
      data.loop(&Xi_Operator::local_apply_cell,
                &Xi_Operator::local_apply_face,
                &Xi_Operator::local_apply_boundary_face,
                this,
                dst,
                src,
                true/*,
...

    {
      ...
      data.cell_loop(&Xi_Operator::local_apply_inverse_mass_matrix,
                     this,
                     dst,
                     dst);
    }
  ...
  // update. It calls Xi_Operator::apply() followed by some updates
  ...
  template <int dim, int degree, int n_points_1d>
  void Xi_Operator<dim, degree, n_points_1d>::perform_stage(
    ...
    LinearAlgebra::distributed::Vector<Number> &      next_ri) const
  {
    ...
    //next_ri = 0;
    {
      ...
      data.cell_loop(
        &Xi_Operator::local_apply_inverse_mass_matrix,
        this,
        next_ri,
        vec_ki,
